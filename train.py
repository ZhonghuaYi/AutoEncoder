...
def train(cfg):
    ...
    logger.info(
        'Start Tensorboard with "tensorboard --logdir={}"'.format(tensorboard_dir))

    if cfg.GLOBAL.VISUAL_AUGMENTATION:
        view_dataset(experiment_dir, cfg)  # 数据增强可视化

    train_dataset = MNIST(cfg=cfg, mode='train')
    ...
    print(f"Training {len(train_dataset)} images with {cfg.GLOBAL.EPOCH_NUM} epochs.")

    num_classes = 10
    # logger.info('number of dataset classes : {}'.format(num_classes))  # 这里默认train val的类别数目一致，就没有分开单独打印

    if rank in [-1, 0]:
        val_dataset = MNIST(cfg=cfg, mode='val')
    ...
    for epoch in range(start_epoch, cfg.GLOBAL.EPOCH_NUM):
        ...
        for step, data in enumerate(data_loader):
            ...
            with autocast(enabled=cfg.GLOBAL.USE_AMP):  # 用于混合精度训练
                ...
                loss = loss_function(pred, images.to(device))['loss']
                if rank != -1:
                    loss *= world_size
        ...
        if epoch % cfg.GLOBAL.VAL_EPOCH_STEP == 0 and rank in [-1, 0]:
            val_performance = evaluate(model=net, data_loader=val_loader, device=device)  # 评估验证集的acc
            logger.info("[epoch {}] accuracy: {}".format(epoch, round(val_performance, 3)))
            tags = ["loss", "accuracy", "learning_rate"]
            tb_writer.add_scalar(tags[0], mean_loss, epoch)
            tb_writer.add_scalar(tags[1], val_performance, epoch)
        ...
        if epoch % cfg.GLOBAL.SAVE_EPOCH_STEP == 0 and rank in [-1, 0]:
            ...
            torch.save(checkpoint, checkpoints_dir / "model-{}.pth.tar".format(epoch))
            if val_performance < pre_performance:
                torch.save(checkpoint, checkpoints_dir / "best.pth.tar".format(epoch))
                pre_acc = val_performance
                early_stop_patience = 0
            else:
