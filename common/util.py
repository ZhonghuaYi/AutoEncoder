...
def init_setting(cfg):
    ...
    experiment_dir = experiment_dir.joinpath(cfg.GLOBAL.EXPERIMENT_NAME + '-' + timestr)
    ...
    setting_dir.mkdir(exist_ok=True)  # 保存设置的目录

    copytree('datasets/', str(setting_dir) + '/datasets/')
    copytree('config/', str(setting_dir) + '/config/')
    copytree('loss/', str(setting_dir) + '/loss/')
    copytree('model/', str(setting_dir) + '/model/')
...
def build_scheduler(optimizer, cfg):
    ...
    if cfg.SCHEDULER.NAME == 'linear_lr':
        lf = lambda x: (1 - x / (epochs - 1)) * (1.0 - cfg.SCHEDULER.LR_LRF) + cfg.SCHEDULER.LR_LRF
        scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lf)  # plot_lr_scheduler(optimizer, scheduler, epochs)

    elif cfg.SCHEDULER.NAME == 'cosine_lr':
        lf = lambda x: ((1 - math.cos(x * math.pi / epochs)) / 2) * (1.0 - cfg.SCHEDULER.LR_LRF) + cfg.SCHEDULER.LR_LRF
        scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lf)  # plot_lr_scheduler(optimizer, scheduler, epochs)

    elif cfg.SCHEDULER.NAME == 'multistep_lr':
        scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=cfg.SCHEDULER.MULTI_STEP,
                                             gamma=cfg.SCHEDULER.MULTI_STEP_GAMMA)

    elif cfg.SCHEDULER.NAME == 'onecycle_lr':
        scheduler = lr_scheduler.OneCycleLR(optimizer, cfg.OPTIMIZER.LEARNING_RATE, epochs + 100, pct_start=0.05,
                                            cycle_momentum=False, anneal_strategy=cfg.SCHEDULER.ONE_CYCLE_TYPE)

    else:
        raise ValueError(f"Specified scheduler '{cfg.SCHEDULER.NAME}' not supported.")
...
def build_optimizer(model, cfg, logger):
    ...
    if cfg.OPTIMIZER.NAME == "Adam":
        optimizer = Adam(g2, lr=cfg.OPTIMIZER.LEARNING_RATE,
                         betas=(cfg.OPTIMIZER.BETA1, cfg.OPTIMIZER.BETA2), eps=cfg.OPTIMIZER.EPSILON)
        # add g1 with weight_decay
        optimizer.add_param_group({'params': g1, 'weight_decay': cfg.OPTIMIZER.WEIGHT_DECAY})
        # add g2 (biases)
        optimizer.add_param_group({'params': g0})

    elif cfg.OPTIMIZER.NAME == "AdamW":
        optimizer = AdamW(g0, lr=cfg.OPTIMIZER.LEARNING_RATE,
                          betas=(cfg.OPTIMIZER.BETA1, cfg.OPTIMIZER.BETA2), eps=cfg.OPTIMIZER.EPSILON)
        # add g1 with weight_decay
        optimizer.add_param_group({'params': g1, 'weight_decay': cfg.OPTIMIZER.WEIGHT_DECAY})
        # add g2 (biases)
